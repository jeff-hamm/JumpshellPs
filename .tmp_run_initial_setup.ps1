...
# Resolve profile path using shared resolver logic from source.
...
function Get-GitInfo {
  param([string]$Path)
  $result = [ordered]@{ path = $Path; inGit = $false; gitRoot = $null; dirty = $false; status = @() }
...
# Enforce setup prompt rule: stop if target git repo has uncommitted changes.
...
# Parse recreate block file sections with fenced content.
$lines = Get-Content -LiteralPath $promptPath
$start = -1
$end = -1
for ($i = 0; $i -lt $lines.Count; $i++) {
  if ($start -lt 0 -and $lines[$i] -eq '## Recreate instructions and user-profile skills') { $start = $i }
  if ($start -ge 0 -and $lines[$i] -eq '## Setup-only references') { $end = $i; break }
}
if ($start -lt 0 -or $end -lt 0 -or $end -le $start) { throw 'Could not locate recreate block boundaries in prompt' }
...
while ($i -lt $end) {
  ...
  if ($line -match '^###\s+(.+)$') {
    ...
    if (-not $isFileHeading) { $i++; continue }

    $j = $i + 1
    while ($j -lt $end -and [string]::IsNullOrWhiteSpace($lines[$j])) { $j++ }
    if ($j -ge $end -or $lines[$j] -notmatch '^`{3,}') { $i++; continue }
    $fence = $lines[$j]
    $k = $j + 1
...
if (-not (Test-Path -LiteralPath $VSCODE_PROFILE)) { New-Item -ItemType Directory -Path $VSCODE_PROFILE -Force | Out-Null }
if (-not (Test-Path -LiteralPath $AGENTS_SKILLS_HOME)) { New-Item -ItemType Directory -Path $AGENTS_SKILLS_HOME -Force | Out-Null }

# If profile is not git, try clone then fallback to init with required .gitignore.
$profileNowGit = Get-GitInfo -Path $VSCODE_PROFILE
if (-not $profileNowGit.inGit) {
  $profileParent = Split-Path -Parent $VSCODE_PROFILE
  $profileLeaf = Split-Path -Leaf $VSCODE_PROFILE
  if (-not (Test-Path -LiteralPath $profileParent)) { New-Item -ItemType Directory -Path $profileParent -Force | Out-Null }

  $cloneTried = $false
  if ((Get-Command git -ErrorAction SilentlyContinue) -and -not (Get-ChildItem -Force -LiteralPath $VSCODE_PROFILE -ErrorAction SilentlyContinue | Select-Object -First 1)) {
    $cloneTried = $true
    Push-Location $profileParent
    try { & git clone https://github.com/jeff-hamm/copilot-instructions "$profileLeaf" 2>$null | Out-Null } finally { Pop-Location }
  }
...
# Install shared common scripts to required destinations.
...
function Ensure-ArrayContains {
  ...
  $contains = $false
  foreach ($item in $arr) { if ([string]$item -ieq $Value) { $contains = $true; break } }
  if (-not $contains) { $arr += $Value; Set-Prop -Obj $Obj -Name $Name -Value $arr }
}
...
# Placeholder expansion across all SKILL.md files under ~/.agents/skills.
